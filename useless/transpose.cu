// copy from: https://leimao.github.io/blog/CUDA-Coalesced-Memory-Access/
// nvcc useless/transpose.cu -o transpose -Xptxas -O0
// nvcc useless/transpose.cu -o transpose -Xptxas -O3  # default
...
template <typename T, size_t BLOCK_SIZE = 32>
__global__ void transpose_read_write_coalesced_bad(T* output_matrix,
                                               T const* input_matrix, size_t M,
                                               size_t N)
{
    // BLOCK_SIZE + 1 for avoiding the shared memory bank conflicts.
...
template <typename T>
void launch_transpose_read_write_coalesced_bad(T* output_matrix,
                                           T const* input_matrix, size_t M,
                                           size_t N, cudaStream_t stream)
{
    constexpr size_t const warp_size{32};
    ...
    dim3 const blocks_per_grid{static_cast<unsigned int>(div_up(N, warp_size)),
                               static_cast<unsigned int>(div_up(M, warp_size))};
    transpose_read_write_coalesced_bad<T, warp_size>
        <<<blocks_per_grid, threads_per_block, 0, stream>>>(output_matrix,
                                                            input_matrix, M, N);
    CHECK_LAST_CUDA_ERROR();
}
...
int main()
{
    ...
    for (size_t m{1}; m <= 64; ++m)
    {
        for (size_t n{1}; n <= 64; ++n)
        {
            ...
            assert(verify_transpose_implementation<float>(
                &launch_transpose_read_write_coalesced_bad<float>, m, n));
        }
    }
    ...
    profile_transpose_implementation<float>(
        &launch_transpose_read_write_coalesced<float>, M, N);
    std::cout << "Transpose Read and Write Coalesced (bad)" << std::endl;
    profile_transpose_implementation<float>(
        &launch_transpose_read_write_coalesced_bad<float>, M, N);
}